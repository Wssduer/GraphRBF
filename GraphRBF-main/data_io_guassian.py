...
def Create_NeighResidue3DPoint(psepos, dist, feature_dir, raw_dir, seqanno, feature_combine, train_list, valid_list,
    ...
    for s, (dataset, seqlist) in enumerate(zip(['train', 'valid', 'test'],
        ...
        for seq in tqdm(seqlist):
            ...
            pos_centroid = pos_centroid - new_center
            ...
            for i in range(len(label)):
                ...
                res_posfeas = res_pos_local / dist
                res_pos_global = pos[neigh_index]
                res_pos_side = pos_side[neigh_index] - res_psepos
                res_pos_centroid = pos_centroid[neigh_index] - res_psepos
                res_feas = feas[neigh_index]
                res_feas_addpos = np.concatenate([res_feas, res_posfeas], axis=1)
                ...
                res_pos_centroid_trans = np.dot(res_pos_centroid, coordinate_system)
            ...
            #
            #     # 使用最近氨基酸的CA作为y
            #     min_distance_index = np.argmin(res_dist[res_dist > 0])
            #     y = pos[min_distance_index]
            #     x = res_psepos
            #     z = pos_side[i]
            #     # 标准化
            ...
            #     coordinate_system = np.column_stack((x, y, z))
            #     res_pos = np.dot(pos[neigh_index], coordinate_system)
            #     res_feas = feas[neigh_index]
            #     res_label = label[i]
            #     res_data = {'node_feas': res_feas.astype('float32'),
            #                 'pos': res_pos.astype('float32'),
            #                 'label': res_label.astype('float32'),
            #                 'neigh_index': neigh_index.astype('int32')}
            #     res_data_pos = {'pos': res_pos.astype('float32')}
            #     seq_data.append(res_data_pos)
            # data_dict[seq] = seq_data
            ...
            #     res_dist = np.sqrt(np.sum((pos - res_psepos) ** 2, axis=1))
            #
            #     # 对距离矩阵进行排序，并选择前k个最小距离的残基
            #     k = dist
            #     k_nearest_indices = np.argsort(res_dist)[:k + 1]  # k是您想要的最近邻数(包含本身)
            #
            #     # 获取最近邻的相对位置、特征和标签
            #     res_pos = pos[k_nearest_indices] - res_psepos
            #     res_dist1 = np.sqrt(np.sum((pos[k_nearest_indices] - res_psepos) ** 2, axis=1))
            #     res_feas = feas[k_nearest_indices]
            #     res_label = label[i]
            #
            #     # 构建局部坐标系
            #     x_axis = res_pos[1]  # 最近的点 a 的方向作为 x 轴
            #     xy_plane = np.cross(x_axis, res_pos[2])  # a 和 b 形成的平面的法向量，作为 y 轴的方向
            #     y_axis = xy_plane / np.linalg.norm(xy_plane)  # 归一化 y 轴
            #
            #     # 计算 z 轴，使用叉乘来确保坐标系右手法则
            #     z_axis = np.cross(x_axis, y_axis)
            #
            #     # 将 k 近邻点映射到局部坐标系下
            #     res_psepos0 = (0, 0, 0)
            #     k_nearest_coords = np.dot(res_pos, np.column_stack([x_axis, y_axis, z_axis]))
            #     res_dist2 = np.sqrt(np.sum((k_nearest_coords - res_psepos0) ** 2, axis=1))
            #     # print('res_pos:', res_pos.shape, res_pos)
            #     # print(('k_nearest_coords:', k_nearest_coords.shape, k_nearest_coords))
            #     # print(res_dist1, res_dist2)
            #
            #     # 创建邻居数据
            #     res_data = {
            ...
            #         'pos': res_pos.astype('float32'),
            #         'label': res_label.astype('float32'),
            #         'neigh_index': k_nearest_indices.astype('int32')
            #     }
            #
            #     seq_data.append(res_data)
            # data_dict[seq] = seq_data
...
def get_pdb_DF(file_path):
    ...
    current_res_id = None  # 添加当前残基的res_id跟踪变量
    ...
    #             continue
    #         atom_count += 1
    #         res_pdb_id = line[22:27]
    #         if res_pdb_id != before_res_pdb_id:
    #             res_count += 1
    #             current_res_id = res_count  # 更新当前残基的res_id
    #         before_res_pdb_id = res_pdb_id
    #         if line[12:16].strip() not in ['N', 'CA', 'C', 'O', 'H']:
    #             is_sidechain = 1
    #         else:
    ...
    #         tmps = pd.Series(
    #             {'ID': atom_count, 'atom': line[12:16].strip(), 'atom_type': atom_type, 'res': res,
    #              'res_id': current_res_id,  # 使用当前残基的res_id,
    #              'xyz': np.array([float(line[30:38]), float(line[38:46]), float(line[46:54])]),
    #              'occupancy': float(line[54:60]),
    #              'B_factor': float(line[60:66]), 'mass': Relative_atomic_mass[atom_type], 'is_sidechain': is_sidechain,
    #              'charge': atom_fea[0], 'num_H': atom_fea[1], 'ring': atom_fea[2]})
    #         tmps = tmps.to_frame().transpose()
    #         if len(res_id_list) == 0:
    #             res_id_list.append(current_res_id)  # 初始化时添加当前残基的res_id
    #         elif res_id_list[-1] != current_res_id:
    #             res_id_list.append(current_res_id)
    #
    #         # pdb_res = pdb_res.append(tmps, ignore_index=True)
    #         # 在代码的最后，将数据帧连接起来
...
def cal_PSSM(ligand, seq_list, pssm_dir, feature_dir):
    ...
    for seqid in seq_list:
        ...
        #             axis_y += 1
        #         axis_x += 1
        #     # PSSM_feas = torch.tensor(feature)
        #     # PSSM_feas[torch.isnan(PSSM_feas)] = 0
        #     # feature = PSSM_feas.numpy()
        #
        #     nor_pssm_dict[file.split('.')[0]] = feature
...
def PDBResidueFeature(seqlist, PDB_DF_dir, feature_dir, ligand, residue_feature_list, feature_combine, atomfea):
    ...
    for seq_id in tqdm(seqlist):
        ...
        # for fea in residue_feature_list:
        #     with open(feature_dir + '/' + ligand + '_{}.pkl'.format(fea), 'rb') as f:
        #         fea_data = pickle.load(f)
        #     print(fea, list(fea_data.values())[0].shape)
        ...
        if torch.isnan(residue_feas_tensor).any():
            # print(f"{seq_id}蛋白质 residue_feas 中包含 NaN 值")
            #
            # # 查找包含NaN值的位置
            # nan_indices = torch.isnan(residue_feas_tensor)
            #
            # # 获取包含NaN值的行和列的索引
            # nan_rows, nan_cols = torch.where(nan_indices)
            #
            # # 打印包含NaN值的行和列的索引
            # for row, col in zip(nan_rows, nan_cols):
            #     print(f"NaN值出现在行 {row.item()} 列 {col.item()}")
            #
            # print('seq_id:', seq_id, 'residue_feas:', residue_feas)
...
if __name__ == '__main__':
    ...
    #train_list1 = test_list[200:]
    #test_list = test_list[:200]
    #train_list = train_list1 + train_list
    #train_list, valid_list = tv_split(train_list, args.tvseed)
    #StatisticsSampleNum(train_list, valid_list, test_list, seqanno)
